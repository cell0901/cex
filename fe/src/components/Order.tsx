...
export function OrderEntry({
  orderSuccessToast
}: { orderSuccessToast: (side: "buy" | "sell") => void }) {
  const [orderType] = useState("Limit");
  const [price, setPrice] = useState('')
  const [quantity, setQuantity] = useState('')
  const [availableBalance, setBalance] = useState(0)
  ...
  useEffect(() => {
    // Decode the token
    const decodedData = jwtDecode(token!) as CustomJwtPayload;

    // Get the user ID (change 'userId' to match your token structure)
    const userId = decodedData.userId;

    SignalingManager.getInstance().registerCallback("BALANCE_UPDATE", (data: any) => {
      console.log("balance", data.balance)
      setBalance(data.balance.balance)
    }, `balance@${userId}`)

    SignalingManager.getInstance().sendMessage({ type: "SUBSCRIBE", params: [`balance@${userId}`] })

    return () => {
      SignalingManager.getInstance().deRegisterCallback("BALANCE_UPDATE", `balance@${userId}`)
    }
  }, [])
        ...
        <div className="space-y-1 rounded-lg bg-[#27282F] px-3 py-3 text-[13px] text-zinc-400">
          <div className="flex justify-between">
            ...
            <span className="text-zinc-200">
              {(Number(price) * Number(quantity)).toFixed(2)} {"USDC"}
            </span>
          </div>
        </div>
      </div>
      <span className="font-mono text-gray-400 text-[11px] flex items-center mt-1">
        <Info className="size-[12px] m-1" /> Self trades are prevented
      </span>
    </div >
  );
}
